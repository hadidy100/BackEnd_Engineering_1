...
CREATE TABLE forums(
    ...
    timestamp TEXT,
    FOREIGN KEY (creator) REFERNECES USERS(id)
);
...
INSERT INTO forums(name,creator) VALUES('mongodb',(select id form users where username ='jon'));
INSERT INTO forums(name,creator) VALUES('Oracle DB',(select id form users where username ='Anas'));
...
(
  ...
  (
    ...
    (select forumId from forums where name = 'redis'),
...
--Thead #2
  ...
  (
    ...
    (
      ...
      (select forumId from forums where name = 'mongodb'),
      DATETIME('now','localtime')
    );
--Thead #3
    INSERT INTO threads
    (
      ...
      (
        ...
        (select forumId from forums where name = 'Oracle DB'),
        DATETIME('now','localtime')
      );
